...
namespace fe { namespace collision {
	...
	HACD::GraphMatrix HACD::createDualGraph(const std::vector<int>& indices) const
	{
		const int nTriangles = indices.size() / 3;

		GraphMatrix graphEdges(nTriangles, std::vector<bool>(nTriangles, false));
	...
	void HACD::halfEdgeCollapse(int v1, int v2, GraphMatrix& graphEdges) const
	{
		int nVertices = graphEdges.size();

		assert((v1 >= 0 && v1 < nVertices) && (v2 >= 0 && v2 < nVertices)
			&& "The indices of the vertices can't be out of range");
	...
	) const
	{
		...
		assert((t1 >= 0 && t1 < nTriangles) && (t2 >= 0 && t2 < nTriangles)
			&& "The indices of the triangles can't be out of range");
