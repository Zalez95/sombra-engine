...
namespace se::collision {
	...
	private:	// Attributes
		...
		std::vector<std::pair<HalfEdgeMesh, FaceNormals>> mConvexMeshes;
	...
	public:		// Functions
		...
		HACD(float maximumConcavity, float epsilon) :
			...
			mAABBSize(0.0f), mAlpha(0.0f), mBeta(0.0f), mGamma(0.01f) {};

		/** @return	the HalfEdgeMeshes and their Normals vectors generated by
		 *			the HACD algorithm */
		const std::vector<std::pair<HalfEdgeMesh, FaceNormals>>&
			getMeshes() const { return mConvexMeshes; };
	...
	private:
		...
		static std::pair<HalfEdgeMesh, FaceNormals> getMeshFromIndices(
